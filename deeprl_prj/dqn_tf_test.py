...
class Qnetwork():
    def __init__(self, args, h_size, num_frames, num_actions, rnn_cell_1, myScope, rnn_cell_2=None):
        ...
        c_init = np.zeros((1, rnn_cell_1.state_size.c), np.float32)
        h_init = np.zeros((1, rnn_cell_1.state_size.h), np.float32)
        state_init = tf.contrib.rnn.LSTMStateTuple(c_init, h_init)
        ...
        if args.bidir:
            ...
            print "====== rnn_cell_1.zero_state(self.batch_size, tf.float32).shape", len(rnn_cell_1.zero_state(self.batch_size, tf.float32)), rnn_cell_1.zero_state(self.batch_size, tf.float32)[0].shape
            self.rnn_outputs, self.rnn_state = tf.nn.dynamic_rnn(\
                cell=rnn_cell_1, inputs=self.convFlat, dtype=tf.float32, \
                initial_state=state_init, scope=myScope+'_rnn')
